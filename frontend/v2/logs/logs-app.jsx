...
function LogsApp() {
  ...
  const load = async () => {
    try {
      const lr = await fetch('/api/logs?level=' + logLevel.toUpperCase() + '&limit=' + logLimit, { credentials: 'include' });
      if (lr.ok) { const d = await lr.json(); setLines(Array.isArray(d) ? d : []); }
      const er = await fetch('/api/errors/recent?min_level=' + errLevel + '&limit=100', { credentials: 'include' });
      if (er.ok) { const d = await er.json(); setErrors(Array.isArray(d) ? d : []); }
      const br = await fetch('/api/buckets', { credentials: 'include' });
  ...
  var levelStyles = {
    ...
    ERROR: { bg: 'rgba(110,79,154,0.08)', color: '#6e4f9a', border: '#6e4f9a' },
    WARN: { bg: 'rgba(212,168,95,0.08)', color: '#d4a85f', border: '#d4a85f' },
    INFO: { bg: 'transparent', color: '#8a8898', border: 'rgba(26,25,34,0.08)' },
  };

  var filteredLines = logLevel === 'all' ? lines : lines.filter(function(l) {
    var t = typeof l === 'string' ? l : (l.text || l.msg || '');
    return t.toUpperCase().indexOf(logLevel.toUpperCase()) >= 0;
  });
    ...
    React.createElement('div', { className: 'lg-page' },
      ...
      React.createElement('div', { className: 'lg-filter' },
        ['all','INFO','WARN','ERROR'].map(function(lv) {
          return React.createElement('button', { key: lv, className: logLevel === lv ? 'on' : '', onClick: function() { setLogLevel(lv); } }, lv === 'all' ? '全部' : lv);
        }),
        React.createElement('input', {
          type: 'number', value: logLimit, min: 20, max: 2000,
          onChange: function(e) { setLogLimit(parseInt(e.target.value) || 200); },
      ...
      React.createElement('div', { style: { fontSize: 11, color: 'var(--ink-4)', margin: '4px 0 8px' } }, '共 ' + lines.length + ' 行'),
      React.createElement('pre', {
        style: { fontFamily: 'var(--mono)', fontSize: 11, lineHeight: 1.6, maxHeight: '40vh', overflow: 'auto', whiteSpace: 'pre-wrap', background: 'var(--bg)', padding: 12, borderRadius: 'var(--r-sm)', border: '0.5px solid var(--line)', color: 'var(--ink)' },
      }, filteredLines.slice(-logLimit).map(function(l) {
      ...
      React.createElement('div', { style: { marginTop: 28 } },
        React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 } },
          ...
          ['I','W','E','F'].map(function(lv) {
            return React.createElement('button', {
              key: lv, className: errLevel === lv ? 'on' : '',
              onClick: function() { setErrLevel(lv); },
          ...
          React.createElement('button', { onClick: clearErrors, style: { fontSize: 11, padding: '4px 12px', borderRadius: 999, border: '0.5px solid var(--line-2)', background: 'var(--paper)', color: 'var(--ink-3)', cursor: 'pointer' } }, '已读 (清除)'),
        ),
        React.createElement('div', { style: { fontSize: 11, color: 'var(--ink-4)', marginBottom: 8 } }, '共 ' + errors.length + ' 条'),
        errors.map(function(err, i) {
          var st = levelStyles[err.level] || levelStyles.INFO;
          return React.createElement('div', {
            key: i,
            style: { background: st.bg, border: '0.5px solid ' + st.border, borderRadius: 'var(--r-sm)', padding: '10px 14px', marginBottom: 8 },
          },
            React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' } },
              React.createElement('span', { style: { fontSize: 10, fontWeight: 600, padding: '2px 6px', borderRadius: 4, background: st.color, color: '#fff', fontFamily: 'var(--mono)' } }, err.level || '?'),
            ...
            React.createElement('button', {
              onClick: function() { copyError(err, i); },
