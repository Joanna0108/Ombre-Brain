...
function SettingsApp() {
  ...
  return ce('div', null,
    ...
    ce('div', { className: 'st-page' },
      ...
      ce('div', { className: 'st-section' }, ce('h3', null, '⓪ 版本 & 更新'), ce('div', { className: 'st-sub' }, 'Version & Update'),
      ...
      ce('div', { className: 'st-section' }, ce('h3', null, '① 我'), ce('div', { className: 'st-sub' }, '个人信息 / Tunnel / 登出'),
        ce('div', { className: 'st-row' },
          ce('label', null, '称呼'), ce('input', { type: 'text', value: humanName, onChange: function(ev) { setHumanName(ev.target.value); }, placeholder: '人类',
            style: { padding: '6px 10px', border: '0.5px solid var(--line-2)', borderRadius: 6, fontSize: 13, background: 'var(--bg)', color: 'var(--ink)', flex: 1 } }),
          ce('button', { className: 'st-btn primary', onClick: async function() { await fetch('/api/settings/human', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: humanName }), credentials: 'include' }); showMsg('已保存'); } }, '保存'),
        ),
        tunnel ? ce('div', { style: { marginTop: 16 } }, ce('div', { className: 'st-sub', style: { marginBottom: 6 } }, 'Cloudflare Tunnel'),
          ce('div', null, '状态：' + (tunnel.running ? '运行中' : (tunnel.configured ? '已配置' : '未配置'))),
          ce('div', { style: { marginTop: 6 } }, tunnel.running
        ...
        ce('div', { style: { marginTop: 16 } }, ce('button', { className: 'st-btn danger', onClick: function() { if (confirm('退出登录？')) { fetch('/auth/logout', { method: 'POST', credentials: 'include' }); location.reload(); } } }, '退出登录')),
      ),

      ce('div', { className: 'st-section' }, ce('h3', null, '② 服务'), ce('div', { className: 'st-sub' }, 'Service Status'),
        status ? ce('div', null,
        ...
        ce('div', { style: { marginTop: 16 } }, ce('div', { className: 'st-sub', style: { marginBottom: 6 } }, '宿主机目录 (Docker)'),
          ce('div', { className: 'st-row' }, ce('input', { type: 'text', value: hostVault, onChange: function(ev) { setHostVault(ev.target.value); }, placeholder: '/Users/you/Obsidian',
              style: { padding: '6px 10px', border: '0.5px solid var(--line-2)', borderRadius: 6, fontSize: 13, background: 'var(--bg)', color: 'var(--ink)', flex: 1 } }),
            ce('button', { className: 'st-btn primary', onClick: async function() { await fetch('/api/host-vault', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ value: hostVault }), credentials: 'include' }); showMsg('已保存'); } }, '保存'),
          ),
        ),
      ),

      ce('div', { className: 'st-section' }, ce('h3', null, '③ 引擎'), ce('div', { className: 'st-sub' }, '脱水 LLM / 向量化 Embedding / 本地模型'),
        ...
        embInfo ? ce('div', { style: { marginBottom: 20 } },
          ...
          ce('div', { style: { marginTop: 6 } }, ce('button', { className: 'st-btn primary', onClick: async function() { var rr = await fetch('/api/embedding/backfill', { method: 'POST', credentials: 'include' }); showMsg(rr.ok ? '补全已启动' : '失败'); } }, '补全缺失向量')),
        ) : null,
        localEmb ? ce('div', null, ce('h4', { style: { fontFamily: 'var(--serif)', fontSize: 14, marginBottom: 4 } }, '本地 Ollama / bge-m3'),
          ...
          localEmb.has_model ? ce('div', { style: { marginTop: 6 } }, ce('button', { className: 'st-btn primary', onClick: async function() { if (!confirm('切换？')) return; await fetch('/api/embedding/migrate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ target_backend: 'ollama', api_format: 'ollama', model: 'bge-m3' }), credentials: 'include' }); showMsg('迁移已启动'); } }, '切换到本地向量')) : null,
        ) : null,
      ),

      ce('div', { className: 'st-section' }, ce('h3', null, '④ 桶行为'), ce('div', { className: 'st-sub' }, 'Breath 采样 / 默认参数'),
        ...
        config ? ce('div', { style: { marginTop: 8 } }, ce('div', { className: 'st-row' }, ce('label', null, '合并阈值'), ce('span', null, config.merge_threshold || '—'))) : null,
      ),

      // ⑤ (new)
      ce('div', { className: 'st-section' }, ce('h3', null, '⑤ 环境变量'), ce('div', { className: 'st-sub' }, 'OMBRE_* 当前值'),
      ...
      // ⑥⑦⑧ (new)
      ce('div', { className: 'st-section' }, ce('h3', null, '⑥ MCP'), ce('div', { className: 'st-sub' }, '端点'),
      ...
      ce('div', { className: 'st-section' }, ce('h3', null, '⑦ GitHub'), ce('div', { className: 'st-sub' }, '备份'),
        github ? ce('div', null, ce('div', { className: 'st-row' }, ce('label', null, '状态'), ce('span', { className: 'st-status ' + (github.configured ? 'on' : 'off') }, github.configured ? '已配置' : '未配置')),
          ce('div', { style: { marginTop: 6 } }, ce('button', { className: 'st-btn primary', onClick: async function() { await fetch('/api/github/sync', { method: 'POST', credentials: 'include' }); showMsg('同步已触发'); } }, '手动同步')),
        ) : null,
      ),
      ce('div', { className: 'st-section' }, ce('h3', null, '⑧ 危险区'), ce('div', { className: 'st-sub' }, '导出'),
        ce('div', null, ce('a', { href: '/api/export', className: 'st-btn primary' }, '导出全部 ZIP')),
      ),
    ),
  );
}
