...
function Sec(title, sub) {
  ...
  return ce('div', { className: 'st-section' },
    ...
    kids.length > 0 ? ce.apply(null, ['div', null].concat(kids)) : null,
  );
}
...
function Row(label) {
  ...
  return ce.apply(null, ['div', { className: 'st-row' }].concat(kids));
}
...
function SettingsApp() {
  ...
  sections.push(Sec('⓪ 版本 & 更新', 'Version & Update',
  ...
  sections.push(Sec('① 我', '个人信息 / Tunnel / 登出',
    Row('称呼',
  ...
  sections.push(Sec('② 服务', 'Service Status',
    status ? ce('div', null,
  ...
  sections.push(Sec('③ 引擎', '脱水 LLM / 向量化 Embedding / 本地模型',
  ...
  sections.push(Sec('④ 桶行为', 'Breath 采样 / 默认参数',
  ...
  sections.push(Sec('⑤ 环境变量', 'OMBRE_* 当前值',
  ...
  sections.push(Sec('⑥ MCP 配置', 'Claude Desktop 端点 · 主 /mcp + 副 /mcp-extra（突破 claude.ai 5 工具上限）',
  ...
  sections.push(Sec('⑦ GitHub 同步', '备份 + 导入恢复',
    github ? ce('div', null,
