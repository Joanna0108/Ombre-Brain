  ...
  if (curTab === '⓪版本') {
    ...
    panel = Sec('③ 引擎', '脱水 LLM / 向量化 Embedding / 本地模型',
      ...
      Row('API Key', Pwd({ value: form.dehyKey || '', onChange: function(ev) { setF('dehyKey', ev.target.value); }, placeholder: dehy.api_key_masked || '留空 = 不修改' }),
        Btn('保存', '', async function() { if (!form.dehyKey) return; await fetch('/api/env-config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ updates: { OMBRE_COMPRESS_API_KEY: form.dehyKey } }), credentials: 'include' }); sm('已保存'); setF('dehyKey', ''); }),
      ...
      Row('API Key', Pwd({ value: form.embKey || '', onChange: function(ev) { setF('embKey', ev.target.value); }, placeholder: '留空 = 不修改' }),
        Btn('保存', '', async function() { if (!form.embKey) return; await fetch('/api/env-config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ updates: { OMBRE_EMBED_API_KEY: form.embKey } }), credentials: 'include' }); sm('已保存'); setF('embKey', ''); }),
